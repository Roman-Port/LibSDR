...
namespace RomanPort.LibSDR.Components.FFT
{
    ...
    public abstract unsafe class FFTProcessorBase<T> : FFTInterface, IFftMutatorSource, IDisposable where T : unmanaged
    {
        ...
        {
            var window = FilterWindowUtil.MakeWindow(WindowType.BlackmanHarris7, FftBufferSize);
        ...
        private void ProcessIncomingBlock(T* ptr)
        {
            ...
            //Calculate FFT
            var fftGain = 10.0f * MathF.Log10(FftBufferSize) / 2;
        ...
        {
            fftBins = FftBins;
            return OffsetPointerToBins(fftSpectrumFinalBufferPtr);
        }
    }
}