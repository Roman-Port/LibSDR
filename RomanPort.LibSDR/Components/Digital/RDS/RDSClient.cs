...
namespace RomanPort.LibSDR.Components.Digital.RDS
{
    ...
    public class RDSClient
    {
        ...
        /// <summary>
        /// Since the PS name is split into 4 frames of two characters, and only sent sequentially, we want to make sure we only update the buffer if we update the correct indexes in the correct
        /// order. This number will only be increased by 1 at a time, for each frame. When it is 4, we know the text is OK and we reset it and set the psName
        /// </summary>
        public int psUpdatedFrameIndex;
